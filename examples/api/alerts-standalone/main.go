// Command alerts-standalone demonstrates GoLens alerting with mode="standalone".
// The alert evaluator runs in its own goroutine, isolated from the main
// Registry loop. This is useful when alert evaluation is expensive or when
// you want to isolate alert processing from metric ingestion.
//
// Run:
//
//	go run examples/api/alerts-standalone/main.go [-mailer]
//
...
func main() {
	...
	// Enable alerts in standalone mode (separate goroutine).
	...
	mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		w.Write([]byte("hello from alerts-standalone\n"))
	})
	...
	go func() {
		log.Println("GoLens (alerts-standalone) listening on :8080")
